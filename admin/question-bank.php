...
<section class="mb-6 bg-white border rounded-2xl p-5 flex items-center justify-between gap-4 flex-wrap">
  <div><div class="text-[12px] text-indigo-600 font-black uppercase">Question Search Index · Admin Only</div><div id="search-index-status" class="text-[13px] text-slate-500 mt-1">กำลังตรวจสอบสถานะ…</div></div>
  <button id="rebuild-search-index" type="button" class="h-10 px-4 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white text-[13px] font-bold">Index Missing</button>
</section>
<script>
document.addEventListener('DOMContentLoaded', async () => {
  const status=document.getElementById('search-index-status'),button=document.getElementById('rebuild-search-index');
  const load=async()=>{const r=await fetch('adaptive-learning-api.php?action=index_status'),d=await r.json();if(!r.ok)throw new Error(d.error||'โหลดสถานะไม่สำเร็จ');const s=d.stats||{};status.textContent=`ทั้งหมด ${s.total||0} · Indexed ${s.indexed||0} · Pending ${s.pending||0} · Failed ${s.failed||0} · Provider: ${d.provider?.available?d.provider.provider:'Fallback only'}`;};
  try{await load();}catch(e){status.textContent=e.message;}
  button?.addEventListener('click',async()=>{button.disabled=true;button.textContent='กำลังจัดทำดัชนี…';try{const r=await fetch('adaptive-learning-api.php',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'reindex',mode:'missing'})}),d=await r.json();if(!r.ok)throw new Error(d.error||'ทำดัชนีไม่สำเร็จ');await load();}catch(e){status.textContent=e.message;}finally{button.disabled=false;button.textContent='Index Missing';}});
});
</script>
<?php endif; ?>
